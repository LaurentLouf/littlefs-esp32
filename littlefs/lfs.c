...
static int lfs_cache_read(lfs_t *lfs, lfs_cache_t *rcache, const lfs_cache_t *pcache,
...
static int lfs_cache_cmp(lfs_t *lfs, lfs_cache_t *rcache, const lfs_cache_t *pcache,
...
static int lfs_cache_crc(lfs_t *lfs, lfs_cache_t *rcache, const lfs_cache_t *pcache,
...
static int lfs_cache_flush(lfs_t *lfs, lfs_cache_t *pcache, lfs_cache_t *rcache) {
    if (pcache->block != 0xffffffff) {
        int err = lfs->cfg->prog(lfs->cfg, pcache->block, pcache->off, pcache->buffer,
                                 lfs->cfg->prog_size);
        if (err) {
            return err;
        }
        ...
        if (rcache) {
            int res = lfs_cache_cmp(lfs, rcache, NULL, pcache->block, pcache->off, pcache->buffer,
                                    lfs->cfg->prog_size);
            if (res < 0) {
                return res;
            }
...
static int lfs_cache_prog(lfs_t *lfs, lfs_cache_t *pcache, lfs_cache_t *rcache, lfs_block_t block,
...
static int lfs_bd_read(lfs_t *lfs, lfs_block_t block, lfs_off_t off, void *buffer,
                       lfs_size_t size) {
...
static int lfs_bd_prog(lfs_t *lfs, lfs_block_t block, lfs_off_t off, const void *buffer,
                       lfs_size_t size) {
    return lfs_cache_prog(lfs, &lfs->pcache, NULL, block, off, buffer, size);
}

static int lfs_bd_cmp(lfs_t *lfs, lfs_block_t block, lfs_off_t off, const void *buffer,
                      lfs_size_t size) {
    return lfs_cache_cmp(lfs, &lfs->rcache, NULL, block, off, buffer, size);
}

static int lfs_bd_crc(lfs_t *lfs, lfs_block_t block, lfs_off_t off, lfs_size_t size,
                      uint32_t *crc) {
...
static int lfs_alloc(lfs_t *lfs, lfs_block_t *block) {
    while (true) {
        ...
        if (lfs->free.ack == 0) {
            ESP_LOGW(TAG, "No more free space %" PRIu32, lfs->free.i + lfs->free.off);
...
static int lfs_dir_fetch(lfs_t *lfs, lfs_dir_t *dir, const lfs_block_t pair[2]) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        lfs_dir_fromle32(&test);
        err =
            lfs_bd_crc(lfs, tpair[i], sizeof(test), (0x7fffffff & test.size) - sizeof(test), &crc);
        if (err) {
            if (err == LFS_ERR_CORRUPT) {
                continue;
            }
    ...
    if (!valid) {
        ESP_LOGE(TAG, "Corrupted dir pair at %" PRIu32 " %" PRIu32, tpair[0], tpair[1]);
        return LFS_ERR_CORRUPT;
    }
...
static int lfs_dir_commit(lfs_t *lfs, lfs_dir_t *dir, const struct lfs_region *regions, int count) {
        ...
        if (true) {
            ...
            while (newoff < (0x7fffffff & dir->d.size) - 4) {
                if (i < count && regions[i].oldoff == oldoff) {
                    ...
                    err = lfs_bd_prog(lfs, dir->pair[0], newoff, regions[i].newdata,
                                      regions[i].newlen);
                    if (err) {
                        if (err == LFS_ERR_CORRUPT) {
                            goto relocate;
    ...
    relocate:
        ...
        ESP_LOGD(TAG, "Bad block at %" PRIu32, dir->pair[0]);
        ...
        if (lfs_paircmp(oldpair, (const lfs_block_t[2]){0, 1}) == 0) {
            ESP_LOGW(TAG, "Superblock %" PRIu32 " has become unwritable", oldpair[0]);
            return LFS_ERR_CORRUPT;
        }
    ...
    if (relocated) {
        ...
        ESP_LOGD(TAG, "Relocating %" PRIu32 " %" PRIu32 " to %" PRIu32 " %" PRIu32, oldpair[0],
                 oldpair[1], dir->pair[0], dir->pair[1]);
        ...
        if (dir->d.size + lfs_entry_size(entry) <= lfs->cfg->block_size) {
            ...
            int err =
                lfs_dir_commit(lfs, dir,
                               (struct lfs_region[]){{entry->off, 0, &entry->d, sizeof(entry->d)},
    ...
    nextname:
        ...
        *path = pathname;
        ...
        while (true) {
            ...
            int res = lfs_bd_cmp(lfs, dir->pair[0], entry->off + 4 + entry->d.elen + entry->d.alen,
                                 pathname, pathlen);
            if (res < 0) {
                return res;
            }
...
int lfs_dir_read(lfs_t *lfs, lfs_dir_t *dir, struct lfs_info *info) {
    ...
    int err = lfs_bd_read(lfs, dir->pair[0], entry.off + 4 + entry.d.elen + entry.d.alen,
...
static int lfs_ctz_find(lfs_t *lfs, lfs_cache_t *rcache, const lfs_cache_t *pcache,
                        lfs_block_t head, lfs_size_t size, lfs_size_t pos, lfs_block_t *block,
                        lfs_off_t *off) {
    if (size == 0) {
        *block = 0xffffffff;
        *off = 0;
...
static int lfs_ctz_extend(lfs_t *lfs, lfs_cache_t *rcache, lfs_cache_t *pcache, lfs_block_t head,
    ...
    relocate:
        ESP_LOGD(TAG, "Bad block at %" PRIu32, nblock);
...
static int lfs_ctz_traverse(lfs_t *lfs, lfs_cache_t *rcache, const lfs_cache_t *pcache,
                            lfs_block_t head, lfs_size_t size, int (*cb)(void *, lfs_block_t),
                            void *data) {
    if (size == 0) {
        return 0;
    }
...
relocate:
    ESP_LOGD(TAG, "Bad block at %" PRIu32, file->block);
...
lfs_ssize_t lfs_file_read(lfs_t *lfs, lfs_file_t *file, void *buffer, lfs_size_t size) {
    ...
    while (nsize > 0) {
        ...
        if (!(file->flags & LFS_F_READING) || file->off == lfs->cfg->block_size) {
            int err = lfs_ctz_find(lfs, &file->cache, NULL, file->head, file->size, file->pos,
                                   &file->block, &file->off);
            if (err) {
                return err;
            }
    ...
    while (nsize > 0) {
        ...
        if (!(file->flags & LFS_F_WRITING) || file->off == lfs->cfg->block_size) {
            if (!(file->flags & LFS_F_WRITING) && file->pos > 0) {
                ...
                int err = lfs_ctz_find(lfs, &file->cache, NULL, file->head, file->size,
            ...
            lfs_alloc_ack(lfs);
            int err = lfs_ctz_extend(lfs, &lfs->rcache, &file->cache, file->block, file->pos,
                                     &file->block, &file->off);
            if (err) {
                file->flags |= LFS_F_ERRED;
                return err;
        ...
        while (true) {
            int err =
                lfs_cache_prog(lfs, &file->cache, &lfs->rcache, file->block, file->off, data, diff);
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
                    goto relocate;
                }
...
int lfs_file_truncate(lfs_t *lfs, lfs_file_t *file, lfs_off_t size) {
    ...
    if (size < oldsize) {
        ...
        err = lfs_ctz_find(lfs, &file->cache, NULL, file->head, file->size, size, &file->head,
                           &(lfs_off_t){0});
        if (err) {
            return err;
        }
...
int lfs_stat(lfs_t *lfs, const char *path, struct lfs_info *info) {
    ...
    if (lfs_paircmp(entry.d.u.dir, lfs->root) == 0) {
        ...
        err = lfs_bd_read(lfs, cwd.pair[0], entry.off + 4 + entry.d.elen + entry.d.alen, info->name,
                          entry.d.nlen);
        if (err) {
            return err;
        }
...
int lfs_format(lfs_t *lfs, const struct lfs_config *cfg) {
    ...
    if (true) {
        ...
        bool valid = false;
        for (int i = 0; i < 2; i++) {
            err = lfs_dir_commit(lfs, &superdir,
...
int lfs_mount(lfs_t *lfs, const struct lfs_config *cfg) {
    ...
    if (true) {
        ...
        if (err || memcmp(superblock.d.magic, "littlefs", 8) != 0) {
            ESP_LOGE(TAG, "Invalid superblock at %d %d", 0, 1);
            err = LFS_ERR_CORRUPT;
            goto cleanup;
        }
        ...
        if ((major_version != LFS_DISK_VERSION_MAJOR || minor_version > LFS_DISK_VERSION_MINOR)) {
            ESP_LOGE(TAG, "Invalid version %d.%d", major_version, minor_version);
            err = LFS_ERR_INVAL;
            goto cleanup;
        }
    ...
    while (true) {
        ...
        while (dir.off + sizeof(entry.d) <= (0x7fffffff & dir.d.size) - 4) {
            ...
            if ((0x70 & entry.d.type) == (0x70 & LFS_TYPE_REG)) {
                err = lfs_ctz_traverse(lfs, &lfs->rcache, NULL, entry.d.u.file.head,
...
static int lfs_relocate(lfs_t *lfs, const lfs_block_t oldpair[2], const lfs_block_t newpair[2]) {
    ...
    if (res) {
        ...
        if (lfs_paircmp(oldpair, lfs->root) == 0) {
            ESP_LOGD(TAG, "Relocating root %" PRIu32 " %" PRIu32, newpair[0], newpair[1]);
...
int lfs_deorphan(lfs_t *lfs) {
    ...
    for (lfs_size_t i = 0; i < lfs->cfg->block_count; i++) {
        ...
        if (!(0x80000000 & pdir.d.size)) {
            ...
            if (!res) {
                // we are an orphan
                ESP_LOGD(TAG, "Found orphan %" PRIu32 " %" PRIu32, pdir.d.tail[0], pdir.d.tail[1]);
            ...
            if (!lfs_pairsync(entry.d.u.dir, pdir.d.tail)) {
                // we have desynced
                ESP_LOGD(TAG, "Found desync %" PRIu32 " %" PRIu32, entry.d.u.dir[0],
                         entry.d.u.dir[1]);
        ...
        while (true) {
            ...
            if (entry.d.type & 0x80) {
                ...
                if (moved) {
                    ESP_LOGD(TAG, "Found move %" PRIu32 " %" PRIu32, entry.d.u.dir[0],
                             entry.d.u.dir[1]);
                    err = lfs_dir_remove(lfs, &cwd, &entry);
                    if (err) {
                        return err;
                    ...
                    ESP_LOGD(TAG, "Found partial move %" PRIu32 " %" PRIu32, entry.d.u.dir[0],
                             entry.d.u.dir[1]);
                    entry.d.type &= ~0x80;
                    err = lfs_dir_update(lfs, &cwd, &entry, NULL);
                    if (err) {
